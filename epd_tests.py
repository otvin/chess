...
def EPD_test(EPD_line, search_depth):

    OPCODE_BESTMOVE = "bm"
    OPCODE_AVOIDMOVE = "am"
    OPCODE_LIST = [OPCODE_BESTMOVE, OPCODE_AVOIDMOVE]

    for opcode in OPCODE_LIST:
        opcode_pos = EPD_line.rfind(opcode)
        if opcode_pos > -1:
            fen = EPD_line[0:opcode_pos]
            fen = fen + "0 1"  # the FENs leave off the halfmove number and number of full moves, but we require them
            curpos = opcode_pos + len(opcode) + 1  # the +1 is for the space
            semicolonpos = EPD_line.find(";", curpos)
            operand = EPD_line[curpos:semicolonpos]
            idpos = EPD_line.find("id", semicolonpos)
            id = EPD_line[idpos+3:]
            id = id.replace("\"","")  # remove quotes
            id = id.replace(";","") # remove trailing semicolon
            break

    board = chessboard.ChessBoard()
    board.load_from_fen(fen)
    chess.CACHE_HI, CACHE_LOW, CACHE_EXACT, NODES = 0, 0, 0, 0
    chess.global_chess_position_move_cache.inserts = 0
    chess.global_chess_position_move_cache.probe_hits = 0
    start_time = datetime.now()
    best_score, best_known_line = chess.negamax_recurse(board, search_depth, -101000, 101000, search_depth, [])
    end_time = datetime.now()
    sanmove = chessmove_list.pretty_print_move(best_known_line[0], is_san=True)

    line_str = ""
    for c in best_known_line:
            line_str += chessmove_list.pretty_print_move(c) + " "

    if opcode == OPCODE_BESTMOVE:
        resultstr = "Expected {} - Best line: {}".format(operand, line_str)
        if sanmove == operand:
            success = True
        else:
            success = False
    elif opcode == OPCODE_AVOIDMOVE:
        resultstr = "Expected to avoid {} - Best line: {}".format(operand, line_str)
        if sanmove == operand:
            success = False
        else:
            success = True
    ...
    if success:
        ...
        resultstr = "Failure! " + resultstr

    print("Position: {:s} completed at {:%Y-%m-%d %H:%M:%S}".format(id, end_time))
    print(resultstr)
    print("Elapsed time: {:s}".format(str(end_time - start_time)))
    print("Nodes: {:d}".format(chess.NODES))
    print("Cache inserts: {:d}, Cache hits: {:d}".format(chess.global_chess_position_move_cache.inserts, chess.global_chess_position_move_cache.probe_hits))
    print("TT Cache exact: {:d}, Cache high: {:d}, Cache low: {:d}".format(chess.CACHE_EXACT, chess.CACHE_HI, chess.CACHE_LOW))
    print("-------------------")
    print()
...
def Bratko_Kopec_test():
    ...
    start_time = datetime.now()
    for line in EPD_list:
        attempts += 1
        try:
            success = EPD_test(line, search_depth)
            if success:
                successes += 1
            else:
                failures += 1
        except BaseException as e:
            print("Error: {} - continuing".format(str(e)))
            errors += 1
    end_time = datetime.now()

    print ("")
    print ("FINAL REPORT")
    print ("Test complete at {:%Y-%m-%d %H:%M:%S}".format(end_time))
    print ("Tests: {:d}  Successes: {:d}  Failures: {:d}  Errors: {:d}  Percentage Right: {:.2%}".format(attempts, successes, failures, errors, (successes/attempts)))
    print ("Total Elapsed Time {:s}".format(str(end_time-start_time)))
