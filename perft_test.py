    ...
    if is_debug:
        ...
        for move in local_move_list:
            ...
            try:
                ...
                print("previous stack: depth %d applied %s to %s" % (depth, chessmove_list.pretty_print_move(move), beore_fen))
                raise
            ...
            if before_fen != after_fen:
                print("%d : %s : %s : resulted in %s : then rolled back to : %s" % (depth, before_fen, chessboard_list.pretty_print_move(move), middle_fen, after_fen))
